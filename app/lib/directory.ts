...
export async function fetchExcersices(values: value) {
  ...
  console.log(`https://musclewiki.com/newapi/exercise/exercises/directory/?${muscles && `muscles=${muscles}`}&${equipment && `equipment=${equipment}`}&${difficulty && `difficulty=${difficulty}`}`)
  try {
